      ...
      <div id="categories" className="collapse-wrapper px-4 overflow-hidden">
        {categories.map(c => (
          <a key={c.url} href={`/categories/${c.url}`} aria-label={c.title}>
            <button className="w-full h-10 rounded-lg bg-none hover:bg-[var(--btn-plain-bg-hover)] active:bg-[var(--btn-plain-bg-active)] transition-all pl-2 hover:pl-3 text-neutral-700 hover:text-[var(--primary)] dark:text-neutral-300 dark:hover:text-[var(--primary)]">
              <div className="flex items-center justify-between relative mr-2">
                ...
                <div className="transition px-2 h-7 ml-4 min-w-[2rem] rounded-lg text-sm font-bold text-[var(--btn-content)] dark:text-[var(--deep-text)] bg-[oklch(0.95_0.025_var(--hue))] dark:bg-[var(--primary)] flex items-center justify-center">
                  {c.article_count}
                </div>
              </div>
            </button>
