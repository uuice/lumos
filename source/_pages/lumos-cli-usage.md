...
- `src/commands/gen.ts` - 数据生成命令
- `src/commands/server.ts` - 服务器启动命令
- `src/commands/build.ts` - 项目构建命令
- `src/commands/assets.ts` - 资源处理命令
- `src/commands/build-css.ts` - CSS 构建命令
- `src/commands/new.ts` - 内容创建命令
- `src/commands/webp.ts` - WebP 图片转换命令
- `src/commands/help.ts` - 帮助命令
- `src/commands/version.ts` - 版本查看命令
